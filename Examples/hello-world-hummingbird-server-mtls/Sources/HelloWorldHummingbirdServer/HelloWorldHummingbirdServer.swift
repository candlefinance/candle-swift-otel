...
enum HelloWorldHummingbirdServer {
    static func main() async throws {
        ...
        let certsDir = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
            .appending(components: "certs")
        let serverCert = certsDir.appendingPathComponent("chain.crt").path(percentEncoded: false)
        let clientCert = certsDir.appendingPathComponent("client.crt").path(percentEncoded: false)
        let clientKey = certsDir.appendingPathComponent("client.key").path(percentEncoded: false)
